...
static mut LED: MaybeUninit<stm32f1xx_hal::gpio::gpioa::PA5<Output<PushPull>>> =
    MaybeUninit::uninit();
static mut INT_PIN: MaybeUninit<stm32f1xx_hal::gpio::gpioc::PC13<Input<Floating>>> =
    MaybeUninit::uninit();

#[interrupt]
fn EXTI15_10() {
    heprintln!("Interrupt triggered");
...
fn main() -> ! {
    let mut p = pac::Peripherals::take().unwrap();
    {
        ...
        led.into_push_pull_output(&mut gpioa.crl);

        let mut int_pin = gpioc.pc13.into_pull_up_input(&mut gpioc.crh);
        int_pin.make_interrupt_source(&mut afio);
        int_pin.trigger_on_edge(&mut p.EXTI, Edge::Falling);
        int_pin.enable_interrupt(&mut p.EXTI);
        int_pin.into_floating_input(&mut gpioc.crh);
    }
